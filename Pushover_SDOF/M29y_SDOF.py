...
mass = [12.86, 12.86, 0] 
Gamma = 1.03
DispShape = [0.977,1.112,0.991,1.054,0.99,0.97,0.8,0.932,0.96,0.95,1.02,0.97,0.86,0.94,1.109,0.98,0.97,0.96,1.0]
nT = 4
nL = 15
...
matLong = [310,320,330,340,350,360,370,380,390,400,410,420,430,440,450]
matTran = [20,30,40,50]

for i in range(nL):
    op.uniaxialMaterial('Pinching4', matLong[i], LePf1, LePd1/(Gamma*DispShape[-1*(i+1)]), LePf2, LePd2/(Gamma*DispShape[-1*(i+1)]), LePf3, LePd3/(Gamma*DispShape[-1*(i+1)]), LePf4, LePd4/(Gamma*DispShape[-1*(i+1)]), 
                            LeNf1, LeNd1/(Gamma*DispShape[-1*(i+1)]), LeNf2, LeNd2/(Gamma*DispShape[-1*(i+1)]), LeNf3, LeNd3/(Gamma*DispShape[-1*(i+1)]), LeNf4, LeNd4/(Gamma*DispShape[-1*(i+1)]), 
...
matTot = 1000
op.uniaxialMaterial('Parallel', matTot, matLong[0], matLong[1], matLong[2], matLong[3], matLong[4], matLong[5], matLong[6], matLong[7], matLong[8],
                    matLong[9], matLong[10], matLong[11], matLong[12], matLong[13], matLong[14], matTran[0], matTran[1],matTran[2],matTran[3])
